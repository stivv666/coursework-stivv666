    ...
    public class ConsoleManager
    {
        ...
        public void Run()
        {
            while (true)
            {
                ...
                Console.WriteLine("6. Видалити транзакцію");
        ...
        private void AddTransactionUi(TransactionType type)
        {
            ...
            {
                throw new FinanceException("Некоректний вибір категорії!");
            }
        ...
        private void DeleteTransactionUi()
        {
            ...
            if (!transactions.Any())
            {
                throw new FinanceException("Транзакцій для видалення немає!");
            }
            ...
            for (int i = 0; i < transactions.Count; i++)
            {
                ...
                var catName = categories.FirstOrDefault(c => c.Id == t.CategoryId)?.Name ?? "Невідомо";
                string type = t.Type == TransactionType.Income ? "+" : "-";

                Console.WriteLine($"{i + 1}. [{t.Date:dd.MM.yyyy}] {catName} | {type}{t.Amount} | {t.Description}");
            }

            Console.Write("\nВведіть номер транзакції для видалення: ");
            if (int.TryParse(Console.ReadLine(), out int index) && index > 0 && index <= transactions.Count)
            {
                ...
                _financeService.DeleteTransaction(selectedTransaction.Id);
            ...
            {
                throw new FinanceException("Некоректний номер транзакції!");
            }
        }
